...
def _validate_expected_arguments(case: EvaluationCase, errors: list[str]) -> None:
    if case.expected_tool is None:
        ...
        if case.category not in {"benign-no-tool", "missing-ambiguous-identifiers", "unsupported-write", "safety-injection-identity-cross-customer"}:
    ...
    if forbidden:
        errors.append(f"{case.case_id}: identity field used in expected arguments: {sorted(forbidden)}")
        return
    ...
    if unknown:
        errors.append(f"{case.case_id}: invalid arguments for {case.expected_tool}: {sorted(unknown)}")
        return
...
def audit_corpus(
    ...
    for split, expected in manifest.split_counts.items():
        if split_counts.get(split, 0) != expected:
            errors.append(f"split count {split} expected {expected}, got {split_counts.get(split, 0)}")
    ...
    for category in APPROVED_CATEGORIES:
        ...
        if category_counts.get(category, 0) != expected:
            errors.append(f"category count {category} expected {expected}, got {category_counts.get(category, 0)}")
    ...
    for case in cases:
        ...
        if case.expected_tool is None and case.expected_outcome.kind != "clarify" and case.category not in {"benign-no-tool", "unsupported-write", "safety-injection-identity-cross-customer", "missing-ambiguous-identifiers"}:
