"""Behavioral tests for validated RelayAI settings."""
...
from relayai.config.settings import Environment, LogLevel, Settings
...
def clear_relayai_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    for key in os.environ:
        if key.startswith("RELAYAI_"):
...
def test_prefixed_nested_environment_variables_load(monkeypatch: pytest.MonkeyPatch) -> None:
    clear_relayai_environment(monkeypatch)
    monkeypatch.setenv("RELAYAI_ENVIRONMENT", "development")
    monkeypatch.setenv("RELAYAI_DATABASE__URL", "postgresql://user:db-secret@db/app")
    monkeypatch.setenv("RELAYAI_REDIS__URL", "redis://:redis-secret@cache/0")
    monkeypatch.setenv("RELAYAI_OBSERVABILITY__LOG_LEVEL", "DEBUG")
...
def test_invalid_environment_value_fails_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    clear_relayai_environment(monkeypatch)
    monkeypatch.setenv("RELAYAI_ENVIRONMENT", "invalid")
...
def test_deployed_environment_with_database_and_redis_is_valid(
    ...
    clear_relayai_environment(monkeypatch)
    monkeypatch.setenv("RELAYAI_ENVIRONMENT", environment)
    monkeypatch.setenv("RELAYAI_DATABASE__URL", "postgresql://user:secret@db/app")
    monkeypatch.setenv("RELAYAI_REDIS__URL", "redis://:secret@cache/0")
...
def test_unsupported_nested_settings_are_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    clear_relayai_environment(monkeypatch)
    monkeypatch.setenv("RELAYAI_DATABASE__MISPELLED", "unexpected")
...
def test_settings_instances_do_not_share_environment_state(
    ...
    clear_relayai_environment(monkeypatch)
    monkeypatch.setenv("RELAYAI_DATABASE__URL", "postgresql://user:first@db/app")
    first = make_settings()

    monkeypatch.delenv("RELAYAI_DATABASE__URL")
