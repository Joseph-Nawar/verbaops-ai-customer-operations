...
**Goal:** Establish the reproducible Python 3.12/uv foundation, validated configuration, trusted identity boundary, FastAPI request infrastructure, local persistence/runtime foundation, and deterministic CI quality/build gate for RelayAI without implementing later-stage application capabilities.
...
- M1A, M1B, M1C, and M1D are accepted/complete; M1E is the active milestone. M1F remains not started.
...
| M1E Engineering Automation | Deterministic locked quality gates, coverage enforcement, pinned GitHub Actions, and local runtime image build; no publishing or deployment | Active milestone |
...
- [ ] Enforce the existing deterministic-core coverage target at 80% without excluding ordinary production modules.
- [ ] Add a read-only, secret-free GitHub Actions quality job with locked uv synchronization, visible Ruff, mypy, pytest/coverage, and pre-commit steps.
- [ ] Add a separate quality-gated local Docker runtime build with no registry login, push, Compose startup, or deployment behavior.
- [ ] Verify the workflow contract locally and preserve all developer Makefile commands.
