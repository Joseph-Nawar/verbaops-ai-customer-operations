...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("length(btrim(dataset_version)) > 0", name="eval_run_dataset_version_non_empty"),
        ...
        sa.CheckConstraint("length(btrim(capability_alias)) > 0", name="eval_run_capability_non_empty"),
        sa.CheckConstraint("length(btrim(prompt_version)) > 0", name="eval_run_prompt_version_non_empty"),
        sa.CheckConstraint("length(btrim(graph_version)) > 0", name="eval_run_graph_version_non_empty"),
        sa.CheckConstraint("length(btrim(tool_schema_version)) > 0", name="eval_run_tool_schema_version_non_empty"),
        sa.CheckConstraint("case_count >= 0", name="eval_run_case_count_non_negative"),
        sa.CheckConstraint("latency_ms IS NULL OR latency_ms >= 0", name="eval_run_latency_non_negative"),
        sa.CheckConstraint("cost_usd IS NULL OR cost_usd >= 0", name="eval_run_cost_non_negative"),
        sa.CheckConstraint("status IN ('running', 'completed', 'failed')", name="eval_run_status_allowed"),
    )
    op.create_table(
        "eval_results",
        sa.Column("id", _UUID, primary_key=True),
        sa.Column("eval_run_id", _UUID, sa.ForeignKey("eval_runs.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("latency_ms", sa.Float, nullable=True),
        sa.Column("cost_usd", sa.Float, nullable=True),
        sa.Column("agent_run_id", _UUID, sa.ForeignKey("agent_runs.id", ondelete="SET NULL"), nullable=True),
        ...
        sa.CheckConstraint("latency_ms IS NULL OR latency_ms >= 0", name="eval_result_latency_non_negative"),
        sa.CheckConstraint("cost_usd IS NULL OR cost_usd >= 0", name="eval_result_cost_non_negative"),
    )
